...
public class AuraStatusEffectOld : I_BaseStatusEffectOld
{
    public EffectPackOld StatusEffect { get; set; }
    public double Meters { get; set; }
    public Team_Target TeamToTarget { get; set; }

    public AuraStatusEffectOld(EffectPackOld statusEffect, Team_Target teamTarget ,double meters)
    {
    ...
    public void Trigger(CharacterTrigger trigger, CharacterManager target, CharacterManager owner, I_StatusEffectWrapper wrapper)
    {
        if(trigger == CharacterTriggers.FRAME)
        {
            ...
            foreach(CharacterManager cm in needToAddTo)
            {
                ...
                {
                    foreach (I_BaseStatusEffectOld bse in StatusEffect.StatusEffect.BaseStatusEffects)
                    {
